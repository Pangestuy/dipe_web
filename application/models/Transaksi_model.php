...
class Transaksi_model extends CI_Model
{
    ...
    function get_detail_id($id)
    {
        ...
        $this->db->where($this->id, $id);
        $this->db->select("*,".$this->table_detail.".HARGA_BARANG,".$this->table_detail.".STATUS_BARANG,")->from($this->table_detail);
        $this->db->join($this->table_BARANG,$this->table_BARANG.".ID_BARANG=".$this->table_detail.".ID_BARANG");
    ...
    function confirm($data,$id){
        $transaksi=$this->db->select("KODE_TRANSAKSI,TOTAL,ID_BARANG")->from($this->table_detail)->where("ID_DETAIL_TRANSAKSI",$id)->get()->row();
        $this->db->query("Update tb_BARANG set STATUS_BARANG=1 where ID_BARANG='".$transaksi->ID_BARANG."'",FALSE);
        $this->db->where("ID_DETAIL_TRANSAKSI",$id);
        $this->db->update($this->table_detail,$data);
        return $transaksi->KODE_TRANSAKSI;
    }
